...
public class KeeperAcconutController extends MobileBaseController{
	...
	public @ResponseBody ResponseOne<ContainerKeeper> loginByPwd(
		...
		String password = RSAUtils.decrypt(privateKey, keeperAccountRequest.getPassword());  	
        ...
        if (request.getRemoteAddr() != null) {
            LogUtil.debug(this.getClass(), "login", "登录管家：%s 登录IP:%s  登录时间:%s", keeper.getUserName(),
            		request.getRemoteAddr(), TimeUtils.getDateFormatString("yyyy-MM-dd hh:mm:ss", new Date()));
        }
	...
	@ApiOperation(value="获取验证码", httpMethod = "POST", response = BaseResponse.class, notes = "验证手机号并发送验证码")
	...
	public @ResponseBody BaseResponse getVerificationCode(
		...
		if(keeper == null){
			...
			try {
				...
				response.setDesc(message("yxkj.kepper.getVerificationCode.fail"));
	            LogUtil.debug(this.getClass(), "getVerificationCode", "获取验证码失败");
	            return response;
			}
			response.setCode(CommonAttributes.SUCCESS);
			response.setDesc(message("yxkj.kepper.getVerificationCode.success"));
	...
	@ApiOperation(value = "用户验证码登录", httpMethod = "POST", response = BaseResponse.class, notes = "验证码登录")
    @ApiResponses({@ApiResponse(code = 200, message = "code描述[0000:登录成功; 0001,0006:登录失败]")})
	...
	@ApiOperation(value = "修改密码", httpMethod = "POST", response = BaseResponse.class, notes = "通过旧密码修改密码")
	...
	public @ResponseBody ResponseOne<ContainerKeeper> updatePwd(
			@ApiParam(name = "请求参数(json)", value = "{cellPhoneNum:手机号,oldPwd:旧密码,newPwd:新密码}", required = true) 
			@RequestBody KeeperAccountRequest keeperAccountRequest, 
    		HttpServletRequest request){
		ResponseOne<ContainerKeeper> response = new ResponseOne<ContainerKeeper>();
		PrivateKey privateKey;
		...
		if (StringUtils.isEmpty(cellPhoneNum)&&StringUtils.isEmpty(newPwd)&&StringUtils.isEmpty(oldPwd)) {
            response.setCode(CommonAttributes.MISSING_REQUIRE_PARAM);
            response.setDesc(message("yxkj.request.param.missing"));
            return response;
        }
		
		if(containerKeeperService.findByCellPhoneNum(cellPhoneNum).getLoginPwd().equals(DigestUtils.md5Hex(oldPwd))){
			try {
			...
			response.setCode(CommonAttributes.FAIL_RESET_PWD);
			response.setDesc(message("yxkj.kepper.password.reset.error"));
	...
	@ApiOperation(value = "忘记密码身份验证", httpMethod = "POST", response = BaseResponse.class, notes = "短信验证码身份验证")
