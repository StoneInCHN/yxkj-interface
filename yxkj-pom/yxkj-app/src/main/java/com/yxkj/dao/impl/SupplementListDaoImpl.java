...
public class SupplementListDaoImpl extends  BaseDaoImpl<SupplementList,Long> implements SupplementListDao {
  ...
  public List<Object[]> findWaitSupplyGoodsDetails(Long suppId, String goodsSn) {
    String jpql = "SELECT s.goodsName, s.sceneName, s.waitSupplyCount FROM SupplementList s "
        + "WHERE s.suppId = :suppId AND s.goodsSn = :goodsSn";
  ...
  public List<Object[]> getWaitSupplyContainerGoods(Long suppId, Long cntrId, int pageNo,
      int pageSize) {
    String sql = "SELECT s.id, c.sn, s.goods_sn, s.goods_name, s.remain_count, s.wait_supply_count FROM t_supp_list s JOIN t_cntr_channel c"
