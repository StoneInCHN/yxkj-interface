...
@RequestMapping("/admin/adResource")
@Api(value = "(货柜后台)广告库管理", description = "广告库管理页面", tags = "AdResourceController")
public class AdResourceController extends BaseController {
  ...
  public @ResponseBody ResponseMultiple<Map<String, Object>> list(@ApiParam(name = "请求参数Json",
      value = "参数[pageNumber:页码；pageSize:总页数;fileType:广告类型]") @RequestBody AdResourceListRequest request) {
    ResponseMultiple<Map<String, Object>> response = new ResponseMultiple<Map<String, Object>>();
    Pageable pageable = new Pageable(request.getPageNumber(), request.getPageSize());

    List<Filter> filters = new ArrayList<>();
    if (request.getFileType() != null) {
      LogUtil.debug(AdMachine.class, "list", "fileType:%s", request.getFileType());
      Filter fileTypeFilter = new Filter("fileType", Filter.Operator.eq, request.getFileType());
