 ...
 *
 */
@Controller("supplementRecordController")
@RequestMapping("/admin/supplementRecord")
@Api(value = "(货柜后台)补货记录", description = "补货记录页面")
public class SupplementRecordController extends BaseController {   
    ...
    @ApiOperation(value = "补货记录列表", httpMethod = "POST", response = ResponseMultiple.class, notes = "补货记录列表")
	...
	public @ResponseBody BaseResponse update(
        ...
        filters.add(Filter.isNotNull("suppSum.suppEndTime"));
        if (request.getSupplementData() != null) {
            ...
            if (supplementData.getStartDate() != null) {
                filters.add(Filter.ge("suppSum.suppStartTime", TimeUtils.formatDate2Day0(supplementData.getStartDate())));
            }
            if (supplementData.getEndDate() != null) {
                filters.add(Filter.lt("suppSum.suppEndTime", TimeUtils.formatDate2Day59(supplementData.getEndDate())));
            }   
  	  	}
        Page<SupplementRecord> page = supplementRecordService.findPage(pageable);      
        String[] propertys =
            {"id", "sceneSn", "sceneName", "cntrSn", "channel.sn", "goodsSn", "goodsName", "suppSum.suppEndTime", "supplyCount", "suppPic.source", "suppName", "suppMobile"};
    ...
    public void dataExport(HttpServletRequest request, HttpServletResponse response) throws IOException {
      ...
      if (StringUtils.isNotBlank(startTime)) {    	  
          filters.add(Filter.ge("suppSum.suppStartTime", TimeUtils.formatDate2Day0(startTime)));
      }
      if (StringUtils.isNotBlank(endTime)) {
          filters.add(Filter.lt("suppSum.suppEndTime", TimeUtils.formatDate2Day59(endTime)));
      }
	
      List<SupplementRecord> lists = supplementRecordService.findList(null, filters, null); 
      
      String title = "Supplement Record List"; // 工作簿标题，同时也是excel文件名前缀
      String[] headers = {"sceneSn", "sceneName", "cntrSn", "channelSn", "goodsSn", "goodsName", "supplyCount", "suppEndTime", "suppPerson", "suppPic"}; // 需要导出的字段
      String[] headersName = {"优享空间编号","优享空间地址","补货货柜","补货货道","商品条码（包含规格）","补货数量","补货时间","补货人","补货照片"}; // 字段对应列的列名
      List<Map<String, String>> mapList = null;
      if (lists != null && lists.size() > 0) {
          mapList = exportHelper.prepareExportSupplement(lists);
